...
// التحقق من سجل المشاهدات (حماية 24 ساعة) const lastView = await AdLog.findOne({ telegramId, adId, viewedAt: { $gt: new Date(Date.now() - 24 * 60 * 60 * 1000) } }); if (lastView) return res.json({ success: false, message: 'لقد شاهدت هذا الإعلان مسبقاً اليوم.' }); const ad = await Ad.findById(adId); if (!ad || ad.budget < ad.costPerView) return res.json({ success: false, message: 'الإعلان غير متاح حالياً.' }); // خصم الرصيد من المعلن وإضافته للمستخدم ad.budget -= ad.costPerView; await ad.save(); await User.findOneAndUpdate({ telegramId }, { $inc: { points: ad.costPerView } }); await AdLog.create({ telegramId, adId }); res.json({ success: true, message: 'تم إضافة المكافأة لرصيدك بنجاح!' }); 

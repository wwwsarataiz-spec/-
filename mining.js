...
router.post('/click', async (req, res) => {
  try {
    ...
    user.balance = (user.balance || 0) + 0.001;
    await user.save();

    res.json({
      success: true,
      message: '✅ تم التعدين بنجاح! +0.001 USDT',
...
router.post('/withdraw-capital', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `✅ تم سحب رأس المال (${inv.amount} USDT)`,
      newBalance: user.balance
    });
  ...
  try {
    ...
    const investments = (user.investments || []).map((inv, index) => {
      ...
      return {
        ...
        canWithdrawCapital: now >= endDate
      };
    });
